...
"$DIR/.venv/bin/pip" install -q "Cavuca[rag] @ git+https://github.com/EstevezCodando/Cavuca.git" pypdf boto3
...
#!/bin/bash
# uso: rodar.sh diaria|semanal|mensal   (flock evita duas coletas simultâneas)
...
0 6 * * *  $DIR/rodar.sh diaria   # flexia-coletor: notícias
